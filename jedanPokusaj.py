...
print(word_labels[48])
print(word_paths[48])
print(len(word_labels))
print(len(word_paths))
...
def createModel():
    input_data=Input(name="input", shape=(64,256,1), dtype='float32')
    ...
    conv_to_rnn_dims = (256 // (2 ** 2), (64 // (2 ** 2)) * 64)
    ...
    # inner=Flatten()(gru_2b)
    # transforms RNN output to character activations:
    y_pred = Dense(90, kernel_initializer='he_normal', activation="softmax",
...
# param_grid = dict(batch_size=[8,16,32,64,128,256], epochs=[20,40,60,80,100])

# grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=-1, cv=3)
# # %%
# grid_result=grid.fit(train_data,train_labels,epochs=20,validation_data=(test_data,test_labels))
# # %%

# print("Best: %f using %s" % (grid_result.best_score_, grid_result.best_params_))

# filename='done_model.sav'
# pickle.dump(model, open(filename, 'wb'))

model.fit(train_data,train_labels,epochs=40,validation_data=(test_data,test_labels))
